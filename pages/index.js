				...
				<div className="container">
					...
					<from className="mb-[2rem] w-full flex items-center">
						<input
							...
							className="w-full h-[5rem] border-[0.1rem] border-theme-coal text-[1.8rem] px-[2rem]"
						></input>
						<button
							type="submit"
					...
					<ul className="grid grid-cols-3 gap-[2rem] grid-auto-rows-[20rem]">
						{items.map((item, key) => (
							<li
								key={key}
							...
							>
								<div className="flex items-center gap-[1rem] mb-[2rem] relative">
									{item?.level && (
										<span className="absolute top-[-4rem] right-[-3rem] bg-theme-red text-white font-bold px-[1rem] py-[0.5rem]">
											{item.level}
										</span>
									)}
									<img
